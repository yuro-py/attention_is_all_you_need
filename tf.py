"""
Minimal, application-independent PyTorch implementation of the Transformer
architecture (Vaswani et al., "Attention Is All You Need").

Implements every block in the canonical diagram:
    Input/Output Embedding -> Positional Encoding
    Encoder stack (Nx): Multi-Head Attention -> Add&Norm -> Feed Forward -> Add&Norm
    Decoder stack (Nx): Masked MHA -> Add&Norm -> Cross MHA -> Add&Norm -> FFN -> Add&Norm
    Linear -> Softmax

Style constraint: no user-defined `class`. Everything is built from plain
functions that construct nn.Module *containers* (nn.Linear, nn.LayerNorm,
nn.Embedding, nn.ModuleDict, nn.ModuleList — all library-provided, never
subclassed) and plain functions that do the forward computation on them.
"""
...
# --------------------------------------------------------------------------
# Full Transformer (embeddings + encoder + decoder + linear + softmax)
# --------------------------------------------------------------------------
